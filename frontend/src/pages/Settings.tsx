...
import { Cpu, SlidersHorizontal, Database, Info, Lightbulb, HardDrive, CheckCircle, AlertCircle, Loader2, Zap, Shield, Server } from 'lucide-react';
...
const Settings: React.FC = () => {
  ...
  const providerIcons: Record<string, React.ReactNode> = {
    ...
    local: <Server className="provider-icon" />
  };
  ...
  const getProviderInfoText = () => {
    ...
    if (provider === 'openai') {
      return '当前使用 <strong>OpenAI 兼容 API</strong> 模式。支持 OpenAI、DeepSeek、通义千问、Ollama 等兼容 OpenAI 接口格式的服务。如需切换 Claude，请点击上方提供方按钮。';
    } else if (provider === 'claude') {
      return '当前使用 <strong>Claude (Anthropic)</strong> 模式。请确保已在 Anthropic 控制台获取有效 API Key。Claude 模型在长文本理解和代码生成方面表现优异。';
    } else if (provider === 'local') {
      return '当前使用 <strong>本地模型</strong> 模式。需要本地部署 Ollama、vLLM 或其他兼容 OpenAI 接口的推理引擎。请确保服务已启动且端口可访问。';
    }
    return '';
  };
      ...
      <main className="main-content">
        ...
        <section className="section-card" aria-labelledby="section-provider">
          ...
          {settings.current_provider === 'local' && (
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label className="form-label" htmlFor="local-model-name">模型名称</label>
              <input
                type="text"
                id="local-model-name"
                className="form-input"
                value={settings.providers.local.model_name || 'llama3'}
